...
# 创建新的模型，该模型以ResNet的输出作为输入，并输出特征向量
...
for i in range(0, len(data), batch_size):
    ...
    batch = tf.image.resize( batch, (224, 224) ) # 调整大小以适应ResNet-152的输入尺寸
    batch = tf.keras.applications.resnet.preprocess_input(batch) # 预处理输入数据
